...
export const Route = createFileRoute("/_authenticated/complaints/")({
  head: () => ({ meta: [{ title: "My Complaints — CivicPulse Nexus" }] }),
...
function MyComplaintsPage() {
  ...
  // Loads all of this citizen's complaints once, then filters accurately in UI.
  const query = useQuery({
    queryKey: ["complaints", "mine", "all"],
    queryFn: () => complaintService.mine({ page: 0, size: 500 }),
  });

  const filteredComplaints = useMemo(() => {
    const keyword = search.trim().toLowerCase();
    const complaints = query.data?.items ?? [];
  ...
  }, [query.data?.items, search, status, priority]);
  ...
  return (
    <>
      <PageHeader
        title="My Complaints"
        description="Track all grievances you have filed with the Municipal Corporation."
        breadcrumbs={[{ label: "Home", to: "/" }, { label: "My Complaints" }]}
        actions={
          <Button asChild>
      ...
      <div className="mx-auto max-w-[1600px] space-y-4 p-6">
        <div className="flex flex-wrap items-center gap-2 rounded-sm border border-border bg-card p-3">
          <div className="relative min-w-[240px] flex-1">
            ...
            <Input
              placeholder="Search reference, title, category, department..."
              value={search}
              onChange={(event) => {
